...
@Injectable()
export class ExpressoServiceProvider {
  stock: StockDetail;
  company: CompanyInfo;
  stockList: Stock[];
  watchList: Stock[];
  ...
  getDataForGraph(ticker) {
    return new Promise((resolve, reject) => {
      this.http.get('/1.0/stock/' + ticker + '/batch?types=quote,news&last=10')
        .toPromise()
  ...
  addToPortfolio(id, quantity, date) {
    return new Promise((resolve, reject) => {
      this.http.get('/stock/createQuotePortfolio/' + id + '/' + quantity + '/' + date)
        .toPromise()
        .then((res) => {
          resolve();
        });
